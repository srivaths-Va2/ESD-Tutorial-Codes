...
Create a multitasking application program to demonstrate creation of concurrently
...
in question 2 can be used. Write application code to test the functionality of task
functions developed. Explain the APIs used to create tasks in FreeRTOS.
...
int main(void)
{
  ...
  myCountingSem01Handle = osSemaphoreNew(30, 30, &myCountingSem01_attributes);
...
void StartGLCD(void *argument)
{
  /* USER CODE BEGIN StartGLCD */
	// Initialising the GLCD display
	GLCD_Init();
	GLCD_SetTextColor(GLCD_TEXT_BLUE);
	GLCD_SetBackColor(GLCD_BG_WHITE);
  /* Infinite loop */
  for(;;)
  {
	  osSemaphoreAcquire(myCountingSem01Handle, osWaitForever);
	  ...
	  char string[20] = "RVCE";
	 GLCD_DisplayString(0, 0, 1, string);
    osDelay(2000);
    osSemaphoreRelease(myCountingSem01Handle);
  }
